...
#define AGRA_LS_GOD 0

#define AGRA_LS_RLS 1

NS_LOG_COMPONENT_DEFINE ("AgraRoutingProtocol");
...
struct DeferredRouteOutputTag: public Tag {
	...
	static TypeId GetTypeId() {
		static TypeId tid =
				TypeId("ns3::agra::DeferredRouteOutputTag").SetParent<Tag>();
		return tid;
	}
...
#define AGRA_MAXJITTER          (HelloInterval.GetSeconds () / 2)
/// Random number between [(-AGRA_MAXJITTER)-AGRA_MAXJITTER] used to jitter HELLO packet transmission.
#define JITTER (Seconds (rvar->GetValue (-AGRA_MAXJITTER, AGRA_MAXJITTER))) 
#define FIRST_JITTER (Seconds (rvar->GetValue (0, AGRA_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4

NS_OBJECT_ENSURE_REGISTERED (RoutingProtocol);

/// UDP Port for AGRA control traffic, not defined by IANA yet
const uint32_t RoutingProtocol::AGRA_PORT = 666;
...
TypeId RoutingProtocol::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::agra::RoutingProtocol").SetParent<Ipv4RoutingProtocol>().AddConstructor<
					...
					EnumValue(AGRA_LS_GOD),
					MakeEnumAccessor(&RoutingProtocol::LocationServiceName),
					MakeEnumChecker(AGRA_LS_GOD, "GOD",
					AGRA_LS_RLS, "RLS")).AddAttribute("PerimeterMode",
					...
					MakeUintegerChecker<uint8_t>()).AddAttribute("locationX",
...
bool RoutingProtocol::RouteInput(Ptr<const Packet> p, const Ipv4Header &header,
	...
	if (m_socketAddresses.empty()) {
		NS_LOG_LOGIC("No agra interfaces");
		return false;
	}
	NS_ASSERT(m_ipv4 != 0);
	NS_ASSERT(p != 0);
	...
	if (m_ipv4->IsDestinationAddress(dst, iif)) {
		...
		TypeHeader tHeader(AGRATYPE_POS);
		packet->RemoveHeader(tHeader);
		if (!tHeader.IsValid()) {
			NS_LOG_DEBUG(
					"AGRA message " << packet->GetUid()
							<< " with unknown type received: " << tHeader.Get()
							<< ". Ignored");
			return false;
		}

		if (tHeader.Get() == AGRATYPE_POS) {
			PositionHeader phdr;
...
bool RoutingProtocol::Forwarding(Ptr<const Packet> packet,
	...
	if (m_neighbors.isNeighbour(dst)) {
		...
		nextHop = m_neighbors.ElectrostaticBestNeighbor(Position, myPos, locationX, locationY, object_radius);
                if (nextHop == Ipv4Address::GetZero()) {
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t interface) {
	...
	if (l3->GetNAddresses(interface) > 1) {
		NS_LOG_WARN(
				"AGRA does not work with more then one address per each interface.");
	}
	...
	NS_ASSERT(socket != 0);
	socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvAGRA, this));
	socket->Bind(InetSocketAddress(Ipv4Address::GetAny(), AGRA_PORT));
...
void RoutingProtocol::RecvAGRA(Ptr<Socket> socket) {
	NS_LOG_FUNCTION(this << socket);
	Address sourceAddress;
	Ptr<Packet> packet = socket->RecvFrom(sourceAddress);

	TypeHeader tHeader(AGRATYPE_HELLO);
	packet->RemoveHeader(tHeader);
	if (!tHeader.IsValid()) {
		NS_LOG_DEBUG(
				"AGRA message " << packet->GetUid()
	...
	if (l3->GetNAddresses((interface) == 1)) {
		...
		if (!socket) {
			...
			socket->SetRecvCallback(
					MakeCallback(&RoutingProtocol::RecvAGRA, this));
			socket->BindToNetDevice(l3->GetNetDevice(interface));
		...
		NS_LOG_LOGIC(
				"AGRA does not work with more then one address per each interface. Ignore added address");
	}
}
...
void RoutingProtocol::NotifyRemoveAddress(uint32_t i,
	...
	if (socket) {
		...
		NS_LOG_LOGIC("Remove address not participating in AGRA operation");
	}
}
...
void RoutingProtocol::SendHello() {
	...
	for (std::map<Ptr<Socket>, Ipv4InterfaceAddress>::const_iterator j =
		...
		socket->SendTo(packet, 0, InetSocketAddress(destination, AGRA_PORT));
...
void RoutingProtocol::Start() {
	//std::cout<<"AGRA protocol has started at node["<<m_ipv4->GetObject<Node>()->GetId()<<"]"<<std::endl;
	NS_LOG_FUNCTION(this);
...
Ptr<Ipv4Route> RoutingProtocol::LoopbackRoute(const Ipv4Header & hdr,
	...
	NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(),
			"Valid AGRA source address not found");
...
//	std::cout<<" Add headers to " <<m_ipv4->GetObject<Node>()->GetId() << ". Protocol="<<(uint32_t) protocol<<std::endl;
	...
	if (m_neighbors.isNeighbour(destination)) {
		...
		nextHop = m_neighbors.BestNeighbor(
				m_locationService->GetPosition(destination), myPos);
	}
...
//				<< m_ipv4->GetAddress(1, 0).GetBroadcast() << std::endl;
	...
	if (m_neighbors.isNeighbour(dst)) {
		...
		std::cout << "We start at Repulsion Mode!" << "[node "
				<< m_ipv4->GetObject<Node>()->GetId() << "] ttl="
				<< (uint32_t) header.GetTtl() << std::endl;
		nextHop = m_neighbors.ElectrostaticBestNeighbor(dstPos, myPos,locationX,locationY,object_radius);
	} else {
		//std::cout << "We start at Greedy Forwarding!" << "[node "
		//		<< m_ipv4->GetObject<Node>()->GetId() << "] ttl="
		//		<< (uint32_t) header.GetTtl() << std::endl;
