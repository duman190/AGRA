...
struct DeferredRouteOutputTag: public Tag {
	...
	static TypeId GetTypeId() {
		static TypeId tid =
				TypeId("ns3::gpsr::DeferredRouteOutputTag").SetParent<Tag>();
		return tid;
	}
...
#define JITTER (Seconds (rvar->GetValue (-GPSR_MAXJITTER, GPSR_MAXJITTER))) 
#define FIRST_JITTER (Seconds (rvar->GetValue (0, GPSR_MAXJITTER))) //first Hello can not be in the past, used only on SetIpv4
...
void RoutingProtocol::NotifyInterfaceUp(uint32_t interface) {
	...
	if (l3->GetNAddresses(interface) > 1) {
		NS_LOG_WARN(
				"GPSR does not work with more then one address per each interface.");
	}
	...
	if (l3->GetNAddresses((interface) == 1)) {
		...
		if (!socket) {
			...
			socket->SetRecvCallback(
					MakeCallback(&RoutingProtocol::RecvGPSR, this));
			socket->BindToNetDevice(l3->GetNetDevice(interface));
		...
		NS_LOG_LOGIC(
				"GPSR does not work with more then one address per each interface. Ignore added address");
	}
}
...
void RoutingProtocol::Start() {
	//std::cout<<"GPSR protocol has started at node["<<m_ipv4->GetObject<Node>()->GetId()<<"]"<<std::endl;
	NS_LOG_FUNCTION(this);
...
Ptr<Ipv4Route> RoutingProtocol::LoopbackRoute(const Ipv4Header & hdr,
	...
	NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(),
			"Valid GPSR source address not found");
